...
## Legal Pages (Part of Web Frontend)
- **Location:** `web/terms-conditions.html` and `web/privacy-policy.html` — deployed alongside the main web frontend.
- **Purpose:** Bilingual (EN/AR) Terms & Conditions and Privacy Policy pages loaded via WebView in the app, allowing content updates without republishing the app.
...
- **Configuration:** Set `EXPO_PUBLIC_LEGAL_PAGES_URL` environment variable to the deployed web frontend base URL (e.g., `https://your-frontend.replit.app`). The app loads `{baseUrl}/terms-conditions.html` and `{baseUrl}/privacy-policy.html`.
- **Deployment:** These pages are deployed automatically with the web frontend — no separate deployment needed.
- **Updating Content:** Edit the HTML files in `web/`, redeploy the web frontend — no app update needed.