      ...
      <ThemedView style={[styles.section, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.settingItem}>
          ...
          <Switch
            ...
            trackColor={{ false: theme.border, true: applyOpacity(theme.primary, '80') }}
            thumbColor={preferences.notifications.emailEnabled ? theme.primary : theme.buttonText}
            ios_backgroundColor={theme.border}
          />
        </View>
        ...
        {preferences.notifications.emailEnabled ? (
          <>
            ...
            <ThemedText style={[styles.subsectionLabel, { color: theme.textSecondary }]}>
              {t('settings.emailSummaryFrequency')}
            </ThemedText>
            
            <Spacer height={Spacing.sm} />

            <View style={styles.frequencyContainer}>
              {(['none', 'daily', 'weekly'] as EmailSummaryFrequency[]).map((freq) => (
                <Pressable
                  key={freq}
                  style={({ pressed }) => [
                    styles.frequencyButton,
                    { 
                      backgroundColor: preferences.notifications.emailSummaryFrequency === freq 
                        ? applyOpacity(theme.primary, '15') 
                        : theme.background,
                      borderColor: preferences.notifications.emailSummaryFrequency === freq 
                        ? theme.primary 
                        : theme.border,
                      opacity: pressed ? 0.8 : 1,
                    },
                  ]}
                  onPress={() => handleEmailFrequencyChange(freq)}
                >
                  <ThemedText style={[
                    styles.frequencyLabel, 
                    { 
                      color: preferences.notifications.emailSummaryFrequency === freq 
                        ? theme.primary 
                        : theme.text 
                    }
                  ]}>
                    {t(`settings.frequency${freq.charAt(0).toUpperCase() + freq.slice(1)}`)}
                  </ThemedText>
                </Pressable>
              ))}
            </View>
        ...
        <ThemedView style={[styles.section, { backgroundColor: theme.surface }]}>
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <ThemedText style={[styles.settingDescription, { color: theme.textSecondary, marginTop: 4 }]}>
                {t('settings.eventNotificationsDesc')}
              </ThemedText>
            </View>
            <DDIcon 
          ...
          {showEventPreferences ? (
            <>
              ...
              {relevantEventTypes.map((eventKey, index) => (
                <View key={eventKey}>
                  {index > 0 ? (
                    <View style={[styles.sectionDivider, { backgroundColor: theme.surfaceSecondary }]} />
                  ) : null}
                  <View style={styles.settingItem}>
                    <View style={{ flex: 1 }}>
                      <ThemedText style={[styles.settingLabel, { color: theme.text }]}>
                        {t(getEventTypeLabel(eventKey))}
                      </ThemedText>
                      <ThemedText style={[styles.settingDescription, { color: theme.textSecondary }]}>
                        {t(getEventTypeDescription(eventKey))}
                      </ThemedText>
                    </View>
                    <Switch
                      value={preferences.notifications.eventPreferences[eventKey]}
                      onValueChange={(enabled) => handleEventPreferenceToggle(eventKey, enabled)}
                      trackColor={{ false: theme.border, true: applyOpacity(theme.primary, '80') }}
                      thumbColor={preferences.notifications.eventPreferences[eventKey] ? theme.primary : theme.buttonText}
                      ios_backgroundColor={theme.border}
                    />
                  </View>
                </View>
